    ...
    for attempt in range(3):
        ...
        while time.time() - start_time < timeout:
            ...
            if token_ready:
                ...
                safe_screenshot(sb, screenshot_path(account_index, f"turnstile-success-{mask_server_id(server_id)}"), result)
                return True
...
def renew_server(sb, server_id: str, server_name: str, account_index: int, result: Dict) -> Dict[str, Any]:
    ...
    safe_screenshot(sb, screenshot_path(account_index, f"08-renew-page-{mask_server_id(server_id)}"), result)
    ...
    time.sleep(5)
    handle_adblocker_popup(sb, account_index, result)
    safe_screenshot(sb, screenshot_path(account_index, f"10-renew-after-click-{mask_server_id(server_id)}"), result)
    ...
    try:
        ...
        safe_screenshot(sb, screenshot_path(account_index, f"11-renew-success-{mask_server_id(server_id)}"), result)
        # 关闭成功弹窗
        try:
